...
const provinces: Province[] = [
  ...
  { id: '7', name_th: 'ลพบุรี', name_en: 'Loburi' },
  ...
  { id: '77', name_th: 'บึงกาฬ', name_en: 'buogkan' }
]
function genCustomer(): Customer[] {
  const data: Customer[] = []
  for (let i = 202; i <= 1200; i++) {
    const favIndex = Math.floor(Math.random() * 4)
    const provinceIndex = Math.floor(Math.random() * 77)
    const age = Math.floor(Math.random() * 100) + 1
    const fav = favs[favIndex].id
    const province = provinces[provinceIndex].id
    const morename = Math.floor(Math.random() * nameList.length)
    const payload: Customer = {
      age: age,
      customer_id: i.toString(),
      customer_name: nameList[i % nameList.length] + ' ' + nameList[morename],
      fav: fav,
      province: province
    }
    data.push(payload)
...
const customers: Customer[] = genCustomer()
export { provinces, customers, favs }
